...
/** Returns true when the anchor's TOML advertises a SEP-6 transfer server. */
function hasSep6(toml: Sep1TomlData): toml is Sep1TomlData & { TRANSFER_SERVER: string } {
  return toml.capabilities.sep6 && !!toml.TRANSFER_SERVER;
}

/**
 * Builds an *indicative* off-ramp estimate for an anchor that advertises SEP-6
 * but no SEP-38 quote server and no SEP-24 transfer server: live USD→fiat
 * reference rate applied to the net USDC after the anchor's own published SEP-6
 * withdraw fee. This is an estimate — the firm rate is confirmed at execution.
 */
async function sep6IndicativeRate(
  anchor: { id: string; name: string },
  toml: Sep1TomlData & { TRANSFER_SERVER: string },
  fiatCode: string,
  corridorId: string,
  amount: string,
  sellAmount: number
): Promise<AnchorRate> {
  const [sep6Config, fxRate] = await Promise.all([
    withTimeout(
      getSep6Info(toml.TRANSFER_SERVER, USDC_ASSET.code),
      PER_ANCHOR_TIMEOUT_MS,
  ...
  const feeFixed = sep6Config.feeFixed;
  const feePercent = sep6Config.feePercent;
  const netUsdc = Math.max(0, sellAmount - feeFixed) * (1 - feePercent / 100);
  const totalReceived = netUsdc * fxRate;
  const effectiveRate = sellAmount > 0 ? totalReceived / sellAmount : 0;

  if (!Number.isFinite(totalReceived) || totalReceived <= 0 || effectiveRate <= 0) {
    throw new Error(`could not derive a SEP-6 estimate for ${fiatCode}`);
  }
...
export async function fetchCorridorRates(
  ...
  await Promise.all(
    anchors.map(async (anchor) => {
      ...
      // own published SEP-6 withdraw fee. Covers anchors (e.g. Cowrie) that
      // advertise only SEP-6, not SEP-24 or SEP-38.
      if (hasSep6(toml)) {
        try {
          rates.push(
